## Function code
decreaseGTsiteStrength <- function(cds, sdSeqStartPosition) {
  ...
  if(nrow(sdExchange) > 0) {
    ...
    sdExchange$downstreamCodonToChange <- sdExchange$upstreamCodonToChange + 3
    ...
    sdList$hbs_a <- lapply(sdList$hbs_seq_search, function(x) {
      ...
      positions_seq <- substr(rep(x, length(positions)),
    ...
    sdList <- sdList[order(sdList$n_hbs, sdList$max_hbs, sdList$sum_hex_abs,
                           decreasing = FALSE), ]
    sdList <- sdList[1, ]
    finalInserts <- data.frame(seq = sdList$code_seq,
                               up_cod = sdList$upstreamCodonToChange,
    ...
    fanFunc[2, (finalInserts[i, 2]:finalInserts[i,
             3])] <- c(substr(finalInserts[i, 1], 1, 3),
